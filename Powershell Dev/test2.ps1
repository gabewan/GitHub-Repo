...
$NonCompObjects = New-Object System.Collections.ArrayList


$WebReport_Objects = Import-Csv '\\hqsbisilon1\filesrv1\its\ITS Desktop Support\GRLEWIS\Profile\Powershell Logs\WebReport.Logs\WebReport_Export_Data.csv' | Where-Object{
        $_.ComputerName -notlike "*VDI*"  -and $_.ComputerName -notlike "*VB*" -and $_.ComputerName -notlike "*VM*" -and $_.ComputerName -notlike "*HQ*" -and $_.ComputerName -notlike "*VBAUD*" -and $_.ComputerName -notlike "*VEW*" -and $_.ComputerName -notlike "*VES*" -and $_.ComputerName -notlike "*VBAS*" -and $_.ComputerName -notlike "*VAP*" -and $_.ComputerName -notlike "*VEAS*" -and $_.ComputerName -notlike "*VEND*" -and $_.ComputerName -notlike "*WINPR*" -and $_.ComputerName -notlike "*VBIN*" }  | Sort-Object -Property ComputerName 

foreach ($Object in $WebReport_Objects) {
    ...
    $Temp2Object = "" | Select-Object 'ComputerName','OperatingSystem'
   ...
   $TempObject.OperatingSystem = $object.OS
    $CompObjects.Add($TempObject) | Out-Null
    } 
    if ($ComputerName -notmatch $NameFormatRegex){
    $Temp2Object.ComputerName = $ComputerName
    $Temp2Object.OperatingSystem = $object.OS
    $NonCompObjects.Add($Temp2Object) | Out-Null

    }
  }

$CompObjects | Export-Csv "$LogPath\Win10Migration_RAW.csv" -Force -NoTypeInformation
$NonCompObjects | Export-Csv "$LogPath\Win10Migration_IncorrectFormat.csv" -Force -NoTypeInformation
...
foreach ($Object in $NonComp){
    $Computername = $Object.Computername
    $OS = $Object.OS
    $TempObject = "" | Select-Object 'ComputerName', 'OS', 'IP','LastRelative', 'Grouping'
