...
#> 
    ...
    param( 
        ...
        [string]$fileName="\\hqsbvnxfile1\global\grlewis\AssetTags\QRTags\QR.png", 
...
$Device_List = Import-Csv -Path "C:\temp\delete22.csv" 
$Device_array = @($Device_List)#$Generated_Codes =  

foreach ($Device in $Device_Array.computername){
    
$DeviceID = ($Device_Array | where {$_.ComputerName -eq $Device}).id
    
$WebReports_List = "https://vmwsbigfix01.nghs.com/webreports?page=SingleComputerReport&ComputerName=NGHS_PRD/$DeviceID&IsTranslated=1"

$WebReports_List

#$IDCapture = $WebReports_List

#New-QR -fileName "\\hqsbvnxfile1\global\grlewis\AssetTags\QRTags\$Device.png" -Message "$IDCapture" 
