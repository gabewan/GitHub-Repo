...
﻿function Get-CorrectPrinterConnection {
    ...
    begin {
        $VMCTXPrinters = $VMCTXPRN_Printers
        $WINPrnPrinters = $WINPRN_Printers
        $VBINPRINTPrinters = $VBINPRINT_Printers
    }
...
$IT_Printer_Connections = $PrinterConnections | where {$_.ComputerName -like "408*"}

$printer_array = New-Object System.Collections.ArrayList

foreach ($object in $IT_Printer_Connections){

$temps = "" | Select-Object 'Computer','Printers','Connection','OldConnections'

if ($object.status -ne "offline"){
    
    $temps.Computer = $object.ComputerName
